...
import { Loader2, CalendarIcon, ChevronsUpDown, Check, PackagePlus } from 'lucide-react';
...
export default function EditPurchaseForm({ purchase, project, materials }: EditPurchaseFormProps) {
  ...
  const form = useForm<z.infer<typeof formSchema>>({
    ...
    defaultValues: {
      ...
      purchaseDate: parseISO(purchase.purchaseDate),
      status: purchase.status,
      discount: purchase.discount || 0,
      totalPaid: purchase.totalPaid || purchase.totalPrice,
    },
  });
  ...
  const onSubmit = (values: z.infer<typeof formSchema>) => {
    ...
    const updatedPurchase: Partial<MaterialPurchase> = {
      ...values,
      totalPrice,
      purchaseDate: values.purchaseDate.toISOString(),
    };
    
    const purchaseDocRef = doc(firestore, 'materialPurchases', purchase.id);
    updateDocumentNonBlocking(purchaseDocRef, updatedPurchase);

    setTimeout(() => {
      toast({ title: 'Purchase Updated', description: 'The material purchase has been updated.' });
      setIsLoading(false);
      router.push(`/dashboard/material-purchases/${purchase.id}`);
    }, 1000);
  };
  ...
  return (
    <Card>
      <CardHeader>
        <CardTitle>Edit Purchase Details</CardTitle>
      </CardHeader>
      <Form {...form}>
