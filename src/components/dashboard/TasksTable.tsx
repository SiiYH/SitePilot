...
export default function TasksTable({ tasks: initialTasks, user, users, viewMode }: TasksTableProps) {
  ...
  const handleRowClick = (task: Task) => {
      ...
      const pathSegments = ['projects', task.projectId, 'tasks', task.id];
  ...
  const renderTaskCard = (task: Task) => {
    ...
    return (
        <Card key={task.id} onClick={() => handleRowClick(task)} className={cn(
            ...
            !task.owner && "bg-yellow-500/5 border-yellow-500/20 dark:bg-yellow-500/10 dark:border-yellow-500/20",
            task.status === 'In Progress' && "bg-blue-500/5 border-blue-500/20 dark:bg-blue-500/10 dark:border-blue-500/20",
            task.status === 'Completed' && "bg-green-500/5 border-green-500/20 dark:bg-green-500/10 dark:border-green-500/20"
        )}>
            <CardHeader>
                    ...
                    <TableBody>
                        {sortedTasks.map(task => {
                            ...
                            <TableRow key={task.id} onClick={() => handleRowClick(task)} className={cn(
                                "cursor-pointer",
                                !task.owner && "bg-yellow-500/5 hover:bg-yellow-500/10 dark:bg-yellow-500/10 dark:hover:bg-yellow-500/15",
                                task.status === 'In Progress' && "bg-blue-500/5 hover:bg-blue-500/10 dark:bg-blue-500/10 dark:hover:bg-blue-500/15",
                                task.status === 'Completed' && "bg-green-500/5 hover:bg-green-500/10 dark:bg-green-500/10 dark:hover:bg-green-500/15"
                            )}>
